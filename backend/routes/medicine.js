...
// Add new medicine
router.post('/', (req, res) => {
  const { name, description, category, dosage, price, stock_quantity } = req.body;
  if (!name) return res.status(400).json({ error: 'Medicine name is required' });
  
  db.query(
    'INSERT INTO medicines (name, description, category, dosage, price, stock_quantity) VALUES (?, ?, ?, ?, ?, ?)', 
    [name, description, category, dosage, price || 0, stock_quantity || 0], 
    (err, result) => {
      if (err) {
...
router.put('/:id', (req, res) => {
  ...
  db.query(
    'UPDATE medicines SET name = ?, description = ?, category = ?, dosage = ?, price = ?, stock_quantity = ? WHERE id = ?',
    [name, description, category, dosage, price, stock_quantity, id],
    (err, result) => {
      if (err) return res.status(500).json({ error: 'DB error' });
